<?php
/**
 * @file SITE-ROOT/admin/page_handler.php
 * @version 2025.08.03.1235.01
 * @description The complete and final backend handler. It is now fully equipped to
 * save the new "Right Column Content" component data.
 */

if (!defined('SITE_ROOT')) { define('SITE_ROOT', realpath(dirname(__DIR__, 4))); }
...
if (empty($action)) { $json_input = file_get_contents('php://input'); $data = json_decode($json_input, true); $action = $data['action'] ?? null; }

switch ($action) {
    case 'load': handle_load(); break;
    case 'save': handle_save(); break;
    case 'delete': handle_delete($data ?? null); break;
...
function handle_save() {
    ...
    if (file_put_contents($page_dir . $revision_filename, json_encode($data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES))) {
        echo json_encode(['success' => true, 'page_name' => $page_name, 'page_list_html' => get_updated_page_list_html()]);
    } else {
...
function handle_delete($data) {
    ...
    function delete_directory($dir) { if (!is_dir($dir)) return false; $files = array_diff(scandir($dir), ['.', '..']); foreach ($files as $file) { (is_dir("$dir/$file")) ? delete_directory("$dir/$file") : unlink("$dir/$file"); } return rmdir($dir); }
    if (delete_directory($page_dir)) { echo json_encode(['success' => true, 'page_list_html' => get_updated_page_list_html()]); } else { echo json_encode(['error' => 'Failed to delete the page directory.']); }
    exit;
}

function get_updated_page_list_html() {
    $pages_dir = SITE_ROOT . '/admin/data/pages/';
    $pages = [];
    if (is_dir($pages_dir)) {
