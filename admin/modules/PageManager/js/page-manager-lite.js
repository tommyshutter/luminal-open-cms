    ...
    document.body.addEventListener('click', (e) => {
        ...
        if (e.target.classList.contains('js-sc-insert')) {
            ...
            if (lastFocusedEditor && code) {
                // Inline shortcodes (icons, tokens) insert tight; block shortcodes get
                // a clean blank line above and below so codes never jam ( ]]][[[ ).
                // Opt-in inline via data-inline on the pill; default = block (gets air).
                const isInline = pill.dataset.inline === '1' || pill.dataset.inline === 'true';
                const start = lastFocusedEditor.selectionStart;
                const end   = lastFocusedEditor.selectionEnd;
                const val   = lastFocusedEditor.value;
                let left  = val.substring(0, start);
                let right = val.substring(end);
                let lead = '', trail = '';
                if (!isInline) {
                    // Normalize any existing whitespace jam right at the caret, then re-air
                    // with exactly one blank line (only where there's neighbouring content).
                    left  = left.replace(/[ \t\r\n]+$/, '');
                    right = right.replace(/^[ \t\r\n]+/, '');
                    lead  = left  === '' ? '' : '\n\n';
                    trail = right === '' ? '' : '\n\n';
                }
                const caretStart = left.length + lead.length;
                const caretEnd   = caretStart + code.length;
                lastFocusedEditor.value = left + lead + code + trail + right;

                // Landing zone: flash-select the inserted code so you SEE where it landed,
                // then collapse the caret to just after it.
                lastFocusedEditor.focus();
                try { lastFocusedEditor.setSelectionRange(caretStart, caretEnd); } catch (_) {}
                try {
                    const lineNo = left.split('\n').length;
                    lastFocusedEditor.scrollTop = Math.max(0, (lineNo - 4) * 18);
                } catch (_) {}
                setTimeout(() => {
                    try { lastFocusedEditor.setSelectionRange(caretEnd, caretEnd); } catch (_) {}
                }, 650);
                ...
                showToast(isInline ? `Inserted: ${code}` : `Inserted ${code} (spaced)`);
            }
        }
    });
