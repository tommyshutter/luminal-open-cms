<?php
/**
 * @file admin/widgets/shortcodes-panel.image-galleries.php
 * @desc Lists saved image galleries with pill Insert / Copy next to each shortcode.
 */
if (!defined('SITE_ROOT')) define('SITE_ROOT', realpath(__DIR__ . '/../..'));
$dir = SITE_ROOT . '/admin/data/galleries/images';
$items = [];
if (is_dir($dir)) {
...
<div>
  <?php if (!$items): ?>
    <div class="muted">No image galleries saved yet.</div>
  <?php else: foreach ($items as $name):
    $sc = '[img-gallery name="'.htmlspecialchars($name,ENT_QUOTES).'"]';
  ?>
